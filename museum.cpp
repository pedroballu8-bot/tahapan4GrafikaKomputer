...
// ============================================================
// COLLISION SYSTEM (AABB sederhana - supaya kamera tidak tembus tembok)
// ============================================================
struct AABB {
...
void buildCollisionBoxes() {
    ...
    // Dinding gedung utama (lantai 1) - lubang pintu dibiarkan terbuka (tidak dimasukkan)
    solidBoxes.push_back({ -10.0f, 10.0f, -6.0f, -5.7f });   // dinding belakang
    solidBoxes.push_back({ -10.0f, -9.7f, -6.0f, 6.0f });    // dinding kiri
    solidBoxes.push_back({ 9.7f, 10.0f, -6.0f, 6.0f });      // dinding kanan
    solidBoxes.push_back({ -10.0f, -1.0f, 5.7f, 6.0f });     // dinding depan - kiri pintu
    solidBoxes.push_back({ 1.0f, 10.0f, 5.7f, 6.0f });       // dinding depan - kanan pintu
    ...
    // Pagar beton keliling (4 sisi, dengan celah gerbang di depan)
    solidBoxes.push_back({ -28.0f, 28.0f, -22.15f, -21.85f });  // belakang
    solidBoxes.push_back({ -28.15f, -27.85f, -22.0f, 22.0f });  // kiri
    solidBoxes.push_back({ 27.85f, 28.15f, -22.0f, 22.0f });    // kanan
    solidBoxes.push_back({ -28.0f, -3.0f, 21.85f, 22.15f });    // depan - kiri gerbang
    solidBoxes.push_back({ 3.0f, 28.0f, 21.85f, 22.15f });      // depan - kanan gerbang
...
public:
    ...
    // Coba pindah per-sumbu (X lalu Z) supaya kamera "meluncur" di sepanjang tembok, bukan berhenti total
    void tryMove(float newX, float newZ) {
        if (!collidesAt(newX, camZ)) {
            camX = newX;
        }
        if (!collidesAt(camX, newZ)) {
            camZ = newZ;
        }
    }
    ...
    void keyboard(unsigned char key, int x, int y) {
        ...
        switch(key) {
            case 'w': case 'W':
                tryMove(camX + dirX * speed, camZ + dirZ * speed);
                break;
            case 's': case 'S':
                tryMove(camX - dirX * speed, camZ - dirZ * speed);
                break;
            case 'a': case 'A':
                tryMove(camX - strafeX * speed, camZ - strafeZ * speed);
                break;
            case 'd': case 'D':
                tryMove(camX + strafeX * speed, camZ + strafeZ * speed);
                break;
